...
App.Converters.TsSql = {
   ...
   */
  generateTsSql(sqlState, parsedTables, selectClause, isSelectEdited) {
    const result = [];
    ...
    // 1. カラムの解析
    const columnDefs = this.parseSelectClauseForTs(selectClause, sqlState, parsedTables);

    // 2. 基本名の決定
    const baseName = this.determineBaseNameForTs(sqlState);
    const typeName = `${baseName}Dto`;

    // 3. 型定義の生成
    result.push({
      path: `models/ts/${typeName}.ts`,
      content: this.generateTsInterface(typeName, columnDefs)
    });
   ...
   */
  parseSelectClauseForTs(selectClause, sqlState, parsedTables) {
        ...
        if (tableState) {
          ...
          if (tableDef) {
            if (colName !== '*') {
              ...
              if (colDef) {
                tsType = App.Utils.TypeMapper.mapPostgresToTsType(colDef.type);
   ...
   */
  determineBaseNameForTs(sqlState) {
    if (sqlState.selectedTables.length === 1) {
      return App.Utils.Common.toPascalCase(sqlState.selectedTables[0].tableName);
    }
    if (sqlState.selectedTables.length > 0) {
      return App.Utils.Common.toPascalCase(sqlState.selectedTables[0].tableName) + "Custom";
    }
    return "CustomQuery";
  },
   ...
   */
  generateTsInterface(typeName, columnDefs) {
    let content = `/**\n * SQL query result DTO\n */\n`;
    content += `export interface ${typeName} {\n`;

    columnDefs.forEach(col => {
      const propName = App.Utils.Common.toCamelCase(col.alias);
...
// Backward compat
window.generateTsSql = App.Converters.TsSql.generateTsSql.bind(App.Converters.TsSql);
